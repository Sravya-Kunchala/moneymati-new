...
export default function GoalCalculatorPage() {
  ...
  const [futureGoalOverride, setFutureGoalOverride] = useState<number | null>(null);

  const inflationAdjustedGoal = Math.round(presentValue * Math.pow(1 + inflation / 100, years));
  const futureGoal = futureGoalOverride ?? inflationAdjustedGoal;

  useEffect(() => {
    setFutureGoalOverride(null);
  }, [presentValue, inflation, years]);

  const futureExisting = Math.round(alreadyHave * Math.pow(1 + expectedReturns / 100, years));
  ...
  const goalTrackMin = presentValue;
  const goalTrackMax = Math.round(presentValue * Math.pow(1 + 0.20, years));

  // Animated counter for monthlySIP
  const [displaySIP, setDisplaySIP] = useState(monthlySIP);
  const animRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (animRef.current) clearTimeout(animRef.current);
    const start = displaySIP;
    const diff = monthlySIP - start;
    const steps = 20;
    let i = 0;
    const tick = () => {
      i++;
    ...
    >
      <style>{`
        ...
        .dark-card-curve {
          ...
          height: 200px;
          border-radius: 50%;
          border: 1px solid rgba(17,212,98,0.08);
          pointer-events: none;
        }
        .dark-card-curve-2 {
          position: absolute;
          ...
          height: 260px;
          border-radius: 50%;
          border: 1px solid rgba(17,212,98,0.05);
          pointer-events: none;
        }
        ...
        @media (max-width:767px) {
          ...
          .goal-split          { flex-direction:column !important; }
          .goal-split > *      { width:100% !important; }
        ...
        >
          ...
          <div className="anim-hero-text" style={{ position: "relative", zIndex: 2, maxWidth: 1200, margin: "0 auto" }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 15, lineHeight: 1.7, maxWidth: 500, marginTop: 16 }}>
              Project your future aspirations. Calculates the inflation-adjusted capital and monthly
              contributions required to reach your milestone.
            </p>
          </div>
        </div>
      ...
      >
        ...
        <div className="anim-calc" style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            ...
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#0d1f0d" }}>Goal Planning</h2>
          </div>

          {/* Split: LEFT sliders / RIGHT dark result */}
          <div className="goal-split" style={{ display: "flex", gap: 20, alignItems: "stretch" }}>

            {/* LEFT — sliders */}
            <div style={{ flex: "1 1 50%", background: "#ffffff", borderRadius: 20, padding: "32px 36px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>

              <GoalSlider
                label="Value of your goal today (Present Value)"
                value={presentValue}
                min={10000}
                max={10000000}
                step={10000}
                onChange={setPresentValue}
                displayValue={formatINR(presentValue)}
                minLabel="₹10K"
                maxLabel="₹1 Cr"
              />
              ...
              {/* Row: Inflation + Expected Returns */}
              ...
              {/* Row: Amount you already have + Goal to be reached */}
              <div className="slider-row" style={{ display: "flex", gap: 24 }}>
                <div style={{ flex: 1 }}>
                  <GoalSlider
                    label="Amount you already have"
                    value={alreadyHave}
                    min={0}
                    max={5000000}
                    step={1000}
                    onChange={setAlreadyHave}
                    displayValue={formatINR(alreadyHave)}
                    minLabel="₹0"
                    maxLabel="₹50L"
                  />
                </div>
                <div style={{ flex: 1 }}>
                  <GoalSlider
                    label="Goal to be reached"
                    value={futureGoal}
                    min={goalTrackMin}
                    max={goalTrackMax}
                    step={10000}
                    onChange={(v) => setFutureGoalOverride(v)}
                    displayValue={formatINR(futureGoal)}
                    minLabel={formatINR(goalTrackMin)}
                    maxLabel={formatINR(goalTrackMax)}
                  />
                </div>
              </div>
            </div>
            ...
            >
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                ...
                {/* Required Monthly Investment — Hero metric */}
                ...
                {/* Total Invested + Estimated Returns row */}
                <div style={{ display: "flex", gap: 24 }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: "0 0 8px", fontSize: 12, color: "rgba(255,255,255,0.45)", fontWeight: 500 }}>Total Invested</p>
                    <p style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#ffffff", letterSpacing: "-0.4px" }}>
                      {formatINR(monthlySIP * months)}
                    </p>
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: "0 0 8px", fontSize: 12, color: "rgba(255,255,255,0.45)", fontWeight: 500 }}>Estimated returns</p>
                    <p style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#ffffff", letterSpacing: "-0.4px" }}>
                      {formatINR(Math.max(0, remainingCorpus - monthlySIP * months))}
                    </p>
                  </div>
                </div>
