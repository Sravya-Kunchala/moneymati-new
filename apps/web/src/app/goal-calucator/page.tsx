...
export default function GoalCalculatorPage() {
  ...
  // Build chart data: year-by-year goal growth vs existing savings growth
  const numYears = Math.max(1, Math.min(Math.round(yr), 15));
  const chartBars = Array.from({ length: numYears }, (_, i) => {
    const y = i + 1;
    const goalVal = pv * Math.pow(1 + inf / 100, y);
    const existingVal = ah * Math.pow(1 + er / 100, y);
    return { year: y, value: goalVal, invested: existingVal };
  });
  ...
  const existingPts = mkPoints(chartBars.map((b) => b.invested));
  const goalPts     = mkPoints(chartBars.map((b) => b.value));
  ...
  return (
    <div className={inter.className} style={{ backgroundColor: "#f5f0e8", minHeight: "100vh", fontFamily: "var(--font-inter), sans-serif" }}>
      <style>{`
        ...
        .goal-input-wrap { display:flex; align-items:center; background:#f0ede8; border-radius:12px; padding:14px 16px; gap:8px; transition:background 0.2s, box-shadow 0.2s; }
        .goal-input-wrap:focus-within { background:#e8e4de; box-shadow:0 0 0 2px rgba(13,61,32,0.15); }
        ...
        .goal-input-wrap input { border:none; outline:none; background:transparent; font-size:15px; font-weight:500; color:#222; width:100%; font-family:var(--font-inter),sans-serif; }
        ...
        .gc-calc-card-link:hover .gc-calc-card { transform:translateY(-2px); box-shadow:0 4px 20px rgba(0,0,0,0.10) !important; }
        ...
        @media (max-width: 767px) {
          ...
          .gc-why-section h2 { font-size:20px !important; }
          ...
          <div className="gc-form-card" style={{ background:"#ffffff",borderRadius:20,padding:"36px 40px",boxShadow:"0 2px 16px rgba(0,0,0,0.07)",maxWidth:720,margin:"0 auto" }}>
            ...
            <div className="goal-field" style={{ marginBottom:20 }}>
              ...
              <div className="goal-input-wrap">
                ...
                <input type="number" value={presentValue} onChange={(e) => { setPresentValue(e.target.value); setSubmitted(false); setDone(false); }} placeholder="50,000" />
              </div>
            </div>
            ...
            <div className="gc-grid-2" style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20 }}>
              <div className="goal-field">
                ...
                <div className="goal-input-wrap">
                  <input type="number" value={years} onChange={(e) => { setYears(e.target.value); setSubmitted(false); setDone(false); }} placeholder="10" />
              ...
              <div className="goal-field">
                ...
                <div className="goal-input-wrap">
                  <span className="pfx">₹</span>
                  <input type="number" value={alreadyHave} onChange={(e) => { setAlreadyHave(e.target.value); setSubmitted(false); setDone(false); }} placeholder="5,000" />
                </div>
              </div>
            </div>
            ...
            <div className="gc-grid-2" style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:24 }}>
              <div className="goal-field">
                ...
                <div className="goal-input-wrap">
                  <input type="number" step={0.1} value={inflation} onChange={(e) => { setInflation(e.target.value); setSubmitted(false); setDone(false); }} placeholder="12" />
              ...
              <div className="goal-field">
                ...
                <div className="goal-input-wrap">
                  <input type="number" step={0.1} value={expectedReturns} onChange={(e) => { setExpectedReturns(e.target.value); setSubmitted(false); setDone(false); }} placeholder="15.1" />
                  <span className="sfx">%</span>
                </div>
              </div>
            </div>
              ...
              <div style={{
                ...
                <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                    ...
                    <span style={{ fontSize:9,fontWeight:700,letterSpacing:"0.7px",textTransform:"uppercase",color:"rgba(255,255,255,0.35)" }}>Goal Projection</span>
                  </div>
                  <div style={{ display:"flex",alignItems:"baseline",gap:5 }}>
                    <span style={{ fontSize:13,fontWeight:800,color:"#11D462",fontVariantNumeric:"tabular-nums",animation:"tickerBlink 1.2s ease-in-out infinite" }}>
                      ₹{Math.round((futureGoal * graphProgress) / 100).toLocaleString("en-IN")}
                    </span>
                ...
                <div style={{ position:"relative",height:40,marginBottom:6 }}>
                  ...
                  <svg style={{ position:"absolute",inset:0,width:"100%",height:"100%",overflow:"visible" }} viewBox={`0 0 ${svgW} ${svgH}`} preserveAspectRatio="none">
                    <defs>
                      <clipPath id="revealClipGoal">
                        <rect x="0" y="0" width={svgW * gp / 100} height={svgH} />
                      </clipPath>
                    </defs>
                    {/* Existing savings line */}
                    {chartBars.length > 1 && (
                      <polyline points={existingPts} fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="1.2"
                        strokeDasharray={svgW * 3} strokeDashoffset={svgW * 3 * (1 - gp / 100)}
                        style={{ transition:"stroke-dashoffset 0.06s linear" }} />
                    )}
                    {/* Goal line */}
                    {chartBars.length > 1 && (
                      <polyline points={goalPts} fill="none" stroke="#11D462" strokeWidth="2"
                        strokeDasharray={svgW * 3} strokeDashoffset={svgW * 3 * (1 - gp / 100)}
                        style={{ transition:"stroke-dashoffset 0.06s linear" }} />
                    )}
                    {/* Glowing dot at head */}
                    {chartBars.length > 1 && graphProgress > 0 && (() => {
                      const pts = goalPts.split(" ");
                ...
                <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",gap:12 }}>
                  <div style={{ display:"flex",gap:10 }}>
                    {([["#11D462","Goal Value"],["rgba(255,255,255,0.2)","Savings"]] as const).map(([color,label]) => (
            ...
            {/* Animated Result */}
            {submitted && (
              <div className="result-card gc-results" style={{ marginTop:24,padding:"20px",background:"#f0faf4",borderRadius:12,border:"1px solid #c3e6d0" }}>
                <div className="gc-results-grid" style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,textAlign:"center" }}>
                  <div>
                    ...
                    <div className="result-value result-value-1 gc-results-value" style={{ fontSize:16,fontWeight:800,color:"#0d3d20" }}>₹{futureGoal.toLocaleString("en-IN")}</div>
                  </div>
                  <div>
                    <div className="gc-results-label" style={{ fontSize:12,color:"#666",marginBottom:4 }}>Corpus Still Needed</div>
                    <div className="result-value result-value-2 gc-results-value" style={{ fontSize:16,fontWeight:800,color:"#0d3d20" }}>₹{remainingCorpus.toLocaleString("en-IN")}</div>
                  </div>
                  <div>
                    <div className="gc-results-label" style={{ fontSize:12,color:"#666",marginBottom:4 }}>Monthly SIP Required</div>
                    <div className="result-value result-value-3 gc-results-value" style={{ fontSize:16,fontWeight:800,color:"#11D462" }}>₹{monthlySIP.toLocaleString("en-IN")}</div>
                  </div>
                </div>
              </div>
            )}
