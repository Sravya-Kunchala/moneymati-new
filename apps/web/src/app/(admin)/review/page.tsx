...
function StatusBadge({ status }: { status: Review["status"] }) {
  ...
  return (
    <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${styles[status]} capitalize`}>
      {status}
    </span>
  );
}
...
export default function ReviewsManager() {
  ...
  const [draft, setDraft] = useState<{ name: string; company: string; testimonial: string; rating: number; avatar: string }>({
    name: "",
    company: "",
    testimonial: "",
    rating: 5,
  ...
  const resetDraft = () => setDraft({ name: "", company: "", testimonial: "", rating: 5, avatar: "" });
  ...
  const fetchReviews = () => {
    fetch("/api/reviews")
      ...
      .then((data) => {
        ...
        // keep dummy reviews visible alongside DB rows
        const merged = [...mapped, ...mockReviews];
        setReviews(merged);
      })
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f4f6f4" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <TopNav />
        <div className="min-h-screen bg-gray-50/50 p-6 font-sans" style={{ flex: 1, overflowY: "auto" }}>
          {/* Header */}
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Average Rating</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Total Reviews</p>
              <div className="flex items-end gap-3">
          ...
          {showModal && (
            <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-[28px] shadow-2xl w-full max-w-4xl p-8 animate-in fade-in zoom-in-95 duration-200 border border-gray-100">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start mb-8">
                  <div className="flex flex-col items-center gap-3">
                    <p className="text-xs font-semibold text-emerald-700 uppercase tracking-[0.2em]">Reviewer Info</p>
                    <label className="w-32 h-36 rounded-2xl border-2 border-dashed border-emerald-300 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-emerald-500 transition-colors bg-emerald-50/40">
                      {draft.avatar ? (
                        <img src={draft.avatar} alt="Avatar preview" className="w-20 h-20 rounded-full object-cover" />
                      ) : (
                        <>
                          ...
                          <span className="text-[10px] text-gray-500 text-center leading-tight">PNG or JPG (Max 5MB)</span>
                        </>
                      )}
                      <input
                        type="file"
                  ...
                  <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-gray-600 uppercase tracking-widest mb-2">Reviewer Name</label>
                      <input
                        value={draft.name}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-gray-600 uppercase tracking-widest mb-2">Occupation/Role</label>
                      <input
                        value={draft.company}
                ...
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs font-semibold text-gray-600 uppercase tracking-widest">Review Content</p>
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        key={`modal-star-${s}`}
                ...
                <div className="mb-8">
                  <label className="block text-xs font-semibold text-gray-700 mb-2">Testimonial Insight</label>
                  <textarea
                ...
                <div className="flex flex-col md:flex-row gap-3">
                  ...
                  <button
                    onClick={() => { setShowModal(false); resetDraft(); }}
