...
export default function CAGRCalculatorPage() {
  ...
  const [maturityAmount, setMaturityAmount] = useState(1200000);
  const [years, setYears] = useState(5);

  const cagr = investment > 0 && years > 0 && maturityAmount > investment
    ? (Math.pow(maturityAmount / investment, 1 / years) - 1) * 100
    : 0;

  const totalReturns = maturityAmount - investment;

  // Animated display for CAGR
  const [displayCAGR, setDisplayCAGR] = useState(cagr);
  const animRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (animRef.current) clearTimeout(animRef.current);
    const start = displayCAGR;
    const diff = cagr - start;
    const steps = 20;
    let i = 0;
    const tick = () => {
      i++;
      const eased = 1 - Math.pow(1 - i / steps, 3);
      setDisplayCAGR(start + diff * eased);
  ...
  return (
    ...
    >
      <style>{`
        ...
        .number-pop       { animation: numberPop  0.35s cubic-bezier(0.34,1.56,0.64,1) both; }
        ...
        @media (max-width:767px) {
          ...
          .calc-split          { flex-direction:column !important; }
          .calc-split > *      { width:100% !important; }
        ...
        >
          ...
          >
            ...
            <h2 style={{ fontSize: 28, fontWeight: 800, color: "#0d1f0d", margin: "12px 0 12px" }}>
              Analyze Your Investment Growth With CAGR
            </h2>
            <p style={{ fontSize: 14, color: "#666", lineHeight: 1.7, maxWidth: 440, margin: "0 auto" }}>
              Compound Annual Growth Rate helps you understand the average annual return on your investments over a period of time.
            </p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="anim-calc" style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            ...
            <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#0d1f0d" }}>LUMPSUM CAGR Calculator</h2>
          </div>

          {/* Split layout */}
          <div className="calc-split" style={{ display: "flex", gap: 20, alignItems: "stretch" }}>
            ...
            <div style={{ flex: "1 1 50%", background: "#ffffff", borderRadius: 20, padding: "32px 36px", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
              ...
              <SliderField
                ...
                displayValue={formatINR(investment)}
              />
              ...
              <SliderField
                ...
                min={investment + 1000}
                max={Math.max(investment * 20, 50000000)}
                step={10000}
                onChange={setMaturityAmount}
                displayValue={formatINR(maturityAmount)}
              />
            ...
            >
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                ...
                {/* Annual Growth Rate — Hero metric */}
                <div style={{ marginBottom: 32 }}>
                  <p style={{ margin: "0 0 4px", fontSize: 13, color: "rgba(255,255,255,0.5)", fontWeight: 500 }}>
                    Annual Growth Rate (CAGR)
                  </p>
                  <p style={{ margin: 0, fontSize: 52, fontWeight: 900, color: "#11D462", letterSpacing: "-2px", lineHeight: 1.1 }}>
                    {isFinite(displayCAGR) ? displayCAGR.toFixed(2) : "0.00"}%
                  </p>
                </div>

                <div style={{ height: 1, background: "rgba(255,255,255,0.08)", marginBottom: 28 }} />

                {/* Initial Investment + Final Value row */}
                <div style={{ display: "flex", gap: 24 }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: "0 0 8px", fontSize: 12, color: "rgba(255,255,255,0.45)", fontWeight: 500 }}>Initial Investment</p>
                    <p style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#ffffff", letterSpacing: "-0.4px" }}>
                      {formatINR(investment)}
                    </p>
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: "0 0 8px", fontSize: 12, color: "rgba(255,255,255,0.45)", fontWeight: 500 }}>Final Value</p>
                    <p style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "#ffffff", letterSpacing: "-0.4px" }}>
        ...
        >
          ...
          <div className="why-grid">
            {[
              {
                title: "What is CAGR?",
                desc: "CAGR (Compound Annual Growth Rate) shows the average yearly growth of your investment over a period of time, assuming returns are compounded annually.",
                cls: "anim-why-card-0",
              ...
              },
              {
                ...
                desc: "It helps you evaluate how well your investments have performed and compare different investment options on an equal footing regardless of investment period.",
                cls: "anim-why-card-1",
              ...
              {
                title: "The Benefits",
                desc: "CAGR simplifies complex returns into a single percentage, making it easier to understand growth and make better, well-informed financial decisions.",
                cls: "anim-why-card-2",
