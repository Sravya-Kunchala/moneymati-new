...
export default function CAGRCalculatorPage() {
  ...
  // Build chart data from lumpsum CAGR over years
  const numYears = Math.max(1, Math.min(Math.round(n(lsMaturityAmount)), 15));
  const chartBars = Array.from({ length: numYears }, (_, i) => {
    const y = i + 1;
    const invested = n(lsInvestment);
    const rate = lsCAGR / 100;
    const value = rate > -1 ? invested * Math.pow(1 + rate, y) : invested;
  ...
  const calcCards = [
    ...
    { title: "Sukanya Samriddhi", desc: "Secure your daughter's future.", cls: "anim-calc-card-3", href: "/sukanya", icon: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" fill="#11D462"/></svg>) },
  ];
  ...
  return (
    <div className={inter.className} style={{ backgroundColor: "#f5f0e8", minHeight: "100vh", fontFamily: "var(--font-inter), sans-serif" }}>
      <style>{`
        ...
        .cagr-field { display:flex; flex-direction:column; gap:8px; }
        .cagr-field label { font-size:13px; font-weight:600; color:#333; }
        .cagr-input-wrap { display:flex; align-items:center; background:#f0ede8; border-radius:12px; padding:14px 16px; gap:8px; transition:background 0.2s, box-shadow 0.2s; }
        .cagr-input-wrap:focus-within { background:#e8e4de; box-shadow:0 0 0 2px rgba(13,61,32,0.15); }
        .cagr-input-wrap .pfx { font-size:15px; color:#555; font-weight:500; }
        .cagr-input-wrap .sfx { font-size:14px; color:#888; font-weight:500; margin-left:auto; }
        .cagr-input-wrap input { border:none; outline:none; background:transparent; font-size:15px; font-weight:500; color:#222; width:100%; font-family:var(--font-inter),sans-serif; }
        .cagr-input-wrap input:disabled { color:#555; }
        .cagr-select-wrap { display:flex; align-items:center; background:#f0ede8; border-radius:12px; padding:14px 16px; gap:8px; position:relative; transition:background 0.2s; }
        .cagr-select-wrap select { border:none; outline:none; background:transparent; font-size:15px; font-weight:500; color:#222; width:100%; font-family:var(--font-inter),sans-serif; appearance:none; cursor:pointer; }
        ...
        .section-label { font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:#0d3d20; margin-bottom:20px; }
        .field-grid-2 { display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:20px; }
        .why-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:20px; }
        .calc-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:16px; }
        .calc-card-link { text-decoration:none; display:block; }
        .calc-card-link:hover > div { box-shadow:0 4px 20px rgba(0,0,0,0.10); transform:translateY(-2px); transition:box-shadow 0.2s, transform 0.2s; }
      ...
      <div className="anim-hero hero-section" style={{ position:"relative", minHeight:"320px", padding:"80px 48px 120px", backgroundColor:"#0d2818" }}>
        ...
        <div className="anim-hero-text" style={{ position:"relative",zIndex:2,maxWidth:"1200px",margin:"0 auto" }}>
          ...
          <p style={{ color:"rgba(255,255,255,0.7)",fontSize:15,lineHeight:1.7,maxWidth:500,marginTop:16 }}>
            Measure your investment performance over time. Understand how your money grows annually with compounded returns.
          </p>
        </div>
      </div>
      ...
      <div className="overlap-card-wrap" style={{ backgroundColor:"#f5f0e8",padding:"0 24px" }}>
        <div style={{ maxWidth:"680px",margin:"-80px auto 0",position:"relative",zIndex:10 }}>
          <div className="anim-overlap-card overlap-card" style={{ background:"#ffffff",borderRadius:20,padding:"40px",textAlign:"center",boxShadow:"0 8px 40px rgba(0,0,0,0.12)" }}>
            <span style={{ fontSize:11,fontWeight:700,letterSpacing:"0.8px",textTransform:"uppercase",color:"#11D462" }}>INVESTMENT ANALYSIS</span>
            <h2 style={{ fontSize:28,fontWeight:800,color:"#0d1f0d",margin:"12px 0 12px" }}>Analyze Your Investment Growth With CAGR</h2>
            <p style={{ fontSize:14,color:"#666",lineHeight:1.7,maxWidth:440,margin:"0 auto" }}>
              Compound Annual Growth Rate (CAGR) helps you understand the average annual return on your investments over a period of time.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="calc-form-card" style={{ background:"#ffffff",borderRadius:20,padding:"36px 40px",boxShadow:"0 2px 16px rgba(0,0,0,0.07)",maxWidth:720,margin:"0 auto" }}>
            ...
            <div className="cagr-field" style={{ marginBottom:20 }}>
              ...
              <div className="cagr-input-wrap">
                ...
                <input type="number" value={lsInvestment} onChange={(e) => { setLsInvestment(e.target.value); setSubmitted(false); setDone(false); }} placeholder="50,000" />
              </div>
            </div>
            ...
            <div className="field-grid-2">
              <div className="cagr-field">
                ...
                <div className="cagr-input-wrap">
                  <input type="number" value={lsMaturityAmount} onChange={(e) => { setLsMaturityAmount(e.target.value); setSubmitted(false); setDone(false); }} placeholder="10" />
              ...
              <div className="cagr-field">
                ...
                <div className="cagr-input-wrap">
                  <span className="pfx">₹</span>
                  <input type="number" value={lsYears} onChange={(e) => { setLsYears(e.target.value); setSubmitted(false); setDone(false); }} placeholder="5,000" />
                </div>
              </div>
            </div>
            ...
            <div className="field-grid-2">
              <div className="cagr-field">
                <label>Investment Amount *</label>
                <div className="cagr-input-wrap">
                  <input type="number" value={ulipInvestment} onChange={(e) => { setUlipInvestment(e.target.value); setSubmitted(false); setDone(false); }} placeholder="0" />
                </div>
              </div>
              <div className="cagr-field">
                <label>Investing Frequency</label>
                <div className="cagr-select-wrap">
                  <select value={ulipFrequency} onChange={(e) => { setUlipFrequency(e.target.value); setSubmitted(false); setDone(false); }}>
            ...
            <div className="cagr-field" style={{ marginBottom:20 }}>
              <label>Number of years</label>
              <div className="cagr-input-wrap">
                <input type="number" value={ulipYears} onChange={(e) => { setUlipYears(e.target.value); setSubmitted(false); setDone(false); }} placeholder="15" />
              </div>
            </div>
            ...
            <div className="field-grid-2">
              <div className="cagr-field">
                ...
                <div className="cagr-input-wrap">
                  <input type="number" value={ulipInterimPV} onChange={(e) => { setUlipInterimPV(e.target.value); setSubmitted(false); setDone(false); }} placeholder="12" />
                </div>
              </div>
              <div className="cagr-field">
                <label>Maturity Amount *</label>
                <div className="cagr-input-wrap">
                  <input type="number" value={ulipMaturityAmount} onChange={(e) => { setUlipMaturityAmount(e.target.value); setSubmitted(false); setDone(false); }} placeholder="0" />
                </div>
              </div>
            </div>
            ...
            <div className="field-grid-2">
              <div className="cagr-field">
                ...
                <div className="cagr-input-wrap">
                  <input type="number" value={ulipYearsAtMaturity} onChange={(e) => { setUlipYearsAtMaturity(e.target.value); setSubmitted(false); setDone(false); }} placeholder="12" />
                </div>
              </div>
              <div className="cagr-field">
                <label>CAGR</label>
              ...
              <div style={{
                ...
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    ...
                    <span style={{ fontSize:9, fontWeight:700, letterSpacing:"0.7px", textTransform:"uppercase", color:"rgba(255,255,255,0.35)" }}>CAGR Projection</span>
                  </div>
                  <div style={{ display:"flex", alignItems:"baseline", gap:5 }}>
                    <span style={{ fontSize:13, fontWeight:800, color:"#11D462", fontVariantNumeric:"tabular-nums", animation:"tickerBlink 1.2s ease-in-out infinite" }}>
                      {(lsCAGR * graphProgress / 100).toFixed(2)}%
                    </span>
              ...
              <div className="result-card" style={{ marginTop:24, padding:"20px", background:"#f0faf4", borderRadius:12, border:"1px solid #c3e6d0" }}>
                <div className="result-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, textAlign:"center" }}>
                  <div>
                    ...
                    <div className="result-value result-value-1" style={{ fontSize:16, fontWeight:800, color:"#11D462" }}>{fmt(lsCAGR)}%</div>
                  </div>
                  <div>
                    <div style={{ fontSize:12, color:"#666", marginBottom:4 }}>ULIP CAGR</div>
                    <div className="result-value result-value-2" style={{ fontSize:16, fontWeight:800, color:"#11D462" }}>{fmt(ulipCAGR)}%</div>
                  </div>
                </div>
              </div>
            )}
